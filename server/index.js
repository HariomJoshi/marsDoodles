...
const reqPlayers = 3;
const gameRooms = {};
function createPlayer(playerId, playerName, points = 0,isAdmin = false, wordGuessed = false) {
  return {
    playerId: playerId,
  ...
  socket.on("joinUser", (data) => {
    ...
    if (socketsInRoom) {
      if (socketsInRoom.has(socket.id)) {
        ...
        console.log(`Socket ID ${socket.id} has JOINED the room ${roomId} as PLAYER`);
